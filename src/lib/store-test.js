...
describe("lib/store", () => {
  ...
  describe("Managing advanced colors", () => {
    it("should set ntp_background", () => {
      ...
      expect(
        store.getState().theme.present.colors.ntp_background
      ).to.deep.equal({ r: 255, g: 0, b: 0 });
    });
    ...
    it("should clear ntp_background", () => {
      ...
      expect(store.getState().theme.present.colors.ntp_background).to.be
        .undefined;
    });
  });
});
